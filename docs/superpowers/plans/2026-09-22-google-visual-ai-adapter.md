...
- [ ] **Step 1: Write failing factory/model/capability tests**
...
- [ ] **Step 2: Run the focused test and verify it fails because Google is still the Phase 1 placeholder.**

- [ ] **Step 3: Implement only the explicit factory selection and dedicated adapter configuration.**

- [ ] **Step 4: Run the focused test and verify that Google, MOCK, and unavailable selection are correct.**
...
- [ ] **Step 1: Write failing tests for source/reference mapping, MIME ordering, output normalization, no-image response, and each canonical error category.**

- [ ] **Step 2: Run the focused tests and verify each fails at the missing mapping or normalization behavior.**

- [ ] **Step 3: Implement the smallest request mapper, response parser, and safe error mapper within the Google adapter.**

- [ ] **Step 4: Run the focused test file and verify it passes with only injected SDK stubs.**
...
- [ ] **Step 1: Write failing tests that assert job-provided target/reference resources become canonical image collections and Google startup reports image conditioning/reference support.**

- [ ] **Step 2: Run those focused tests and verify the current singular request contract lacks the collection fields.**

- [ ] **Step 3: Preserve ownership queries and storage loading while passing canonical source/reference collections; add the editing capability only to safe observability where already represented.**

- [ ] **Step 4: Re-run the focused job and observability tests.**
...
- [ ] **Step 1: Run each focused adapter and Visual AI regression suite with network-free stubs.**
- [ ] **Step 2: Run relevant syntax/build checks and `git diff --check`.**
