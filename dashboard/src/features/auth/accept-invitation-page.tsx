...
export function AcceptInvitationPage() {
  ...
  return <PageFrame>
    ...
    <form className="mt-7 space-y-5" onSubmit={submit} noValidate>
      <label className="block text-sm font-medium text-stone-100">Password<input aria-label="Password" type="password" autoComplete="new-password" value={password} onChange={(event) => setPassword(event.target.value)} className="mt-2 block w-full rounded-xl border border-white/20 bg-black/15 px-4 py-3 text-white outline-none transition focus:border-signal" /></label>
      <label className="block text-sm font-medium text-stone-100">Confirm password<input aria-label="Confirm password" type="password" autoComplete="new-password" value={confirmation} onChange={(event) => setConfirmation(event.target.value)} className="mt-2 block w-full rounded-xl border border-white/20 bg-black/15 px-4 py-3 text-white outline-none transition focus:border-signal" /></label>
...
function PageFrame({ children }: { children: React.ReactNode }) {
  return <main className="grid min-h-screen place-items-center bg-[#0B0B0D] px-5 py-8 text-white"><section className="w-full max-w-md rounded-2xl border border-white/15 bg-[#161618]/90 p-6 shadow-[0_22px_70px_rgba(0,0,0,0.36)] sm:p-8">{children}</section></main>;
}
