...
const defaults: GuideExperienceData = {
  ...
  interactive_tool: { enabled: true, title: "Planning snapshot", description: "Capture your planning scope to share with the assistant.", currency: "", pricing_mode: "QUOTE_REQUIRED", approved_pricing_source: "", result_label: "Your planning scope", fields: [{ id: "budget", label: "Indicative budget", description: "", input_type: "NUMBER", required: false, options: [], min: 0, max: 100000000, unit: "" }], calculation: { base_amount: 0, terms: [{ field_id: "budget", kind: "NUMBER_MULTIPLIER", multiplier: 1 }] } },
};
      ...
      <div className="grid gap-6 xl:grid-cols-[1.15fr_.85fr]">
        ...
        >
          ...
          <section className="space-y-4 rounded-xl border border-line p-4" aria-label="Interactive Tool configuration">
            ...
            {draft.interactive_tool.pricing_mode === "APPROVED_PRICING" ? <DashboardField label="Approved pricing source" helper="For example: approved tenant rate card. This is not shown to public visitors."><DashboardInput value={draft.interactive_tool.approved_pricing_source ?? ""} onChange={(event) => setDraft((current) => ({ ...current, interactive_tool: { ...current.interactive_tool, approved_pricing_source: event.target.value } }))} /></DashboardField> : <p className="text-xs text-stone-400">The tool will capture the visitor’s scope and selected requirements. It will not display a monetary estimate until approved pricing is configured.</p>}
