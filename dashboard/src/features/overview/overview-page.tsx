...
export function OverviewPage() {
  ...
  return <div className="mx-auto max-w-[1540px] space-y-4 pb-6">
    ...
    <section className="dashboard-card relative overflow-hidden p-4 sm:p-5"><div className="pointer-events-none absolute inset-y-0 left-0 w-40 bg-[radial-gradient(circle_at_20%_50%,rgba(212,33,41,.16),transparent_72%)]" /><div className="relative grid gap-4 md:grid-cols-[1.45fr_repeat(4,1fr)]"><div className="flex items-center gap-3 border-b border-line/80 pb-4 md:border-b-0 md:border-r md:pb-0"><NeuralBrain /><div><p className="text-sm font-semibold text-ink">AI Insights</p><p className="mt-1 max-w-[15rem] text-xs text-stone-400">{data.insights.best_channel ? channelName(data.insights.best_channel) + ' is currently the highest-volume channel.' : 'More activity is needed to generate reliable insights.'}</p></div></div>{[[Clock3, 'Peak Time', peakTime, peakTime ? 'Most active hour' : '—'], [MessagesSquare, 'Best Channel', channelName(data.insights.best_channel), data.insights.best_channel ? 'Highest conversation volume' : '—'], [Bot, 'Top Performing Bot', data.insights.top_assistant, data.insights.top_assistant ? 'Activity volume' : '—'], [Sparkles, 'Growth', data.insights.growth === null ? null : data.insights.growth + '%', 'vs previous period']].map(([Icon, label, value, detail]) => <div key={String(label)} className="flex items-center gap-3 border-b border-line/80 pb-4 md:border-b-0 md:border-r md:pb-0 last:border-0"><span className="icon-orb h-9 w-9 text-stone-300"><Icon size={17} /></span><div className="min-w-0"><p className="text-[10px] uppercase tracking-[.14em] text-stone-500">{label}</p><p className="mt-1 truncate text-sm font-semibold text-ink">{value || '—'}</p><p className="mt-0.5 text-[10px] text-stone-500">{detail}</p></div></div>)}</div></section>
  </div>;
}
