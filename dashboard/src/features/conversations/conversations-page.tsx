...
import { Bot, ChevronLeft, ChevronRight, Download, ExternalLink, FileText, Headphones, Image as ImageIcon, MessageSquareText, Search, Send, X } from 'lucide-react';
...
export function ConversationsPage() {
  ...
  const conversationsQuery = useQuery({
    queryKey: [...tenantKeys.conversations(tenantId, pageSize, offset), channelContext.type, search],
    queryFn: () => tenantApi.listConversations(tenantId, { limit: pageSize, offset }, { channelType: channelContext.type, search }),
  ...
  return <div className="space-y-4">
    <header className="flex items-center justify-between gap-4"><div><p className="eyebrow">Live customer inbox</p><h1 className="page-title mt-1">Conversations <span className="text-stone-500">/</span> {channelContext.label}</h1></div><LiveState state={liveState} /></header>
    <div className="grid h-[calc(100vh-12.5rem)] min-h-[40rem] gap-3 xl:grid-cols-[minmax(17rem,22rem)_minmax(0,1fr)_minmax(18rem,23rem)]">
      <section className={'panel flex min-h-0 flex-col overflow-hidden ' + (conversationId ? 'hidden xl:flex' : '')}>
        <header className="border-b border-line px-4 py-4"><div className="flex items-center justify-between gap-3"><div><p className="font-semibold text-ink">{channelContext.label}</p><p className="mt-1 text-xs text-stone-400">Most recent activity first</p></div><span className="rounded border border-white/10 px-2 py-1 text-[10px] text-stone-400">{conversations.length}</span></div><label className="relative mt-4 block"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-stone-500" size={15} /><input value={search} onChange={(event) => { setSearch(event.target.value); setOffset(0); }} className="field w-full !py-2 !pl-9 text-xs" placeholder="Search conversations" aria-label="Search conversations" /></label></header>
        <div className="min-h-0 flex-1 overflow-y-auto">{conversations.length === 0 ? <div className="p-5"><EmptyState title="No conversations yet" description="Connected channel traffic will appear here." icon={<MessageSquareText size={20} />} /></div> : <ul className="divide-y divide-line">{conversations.map((item) => <li key={item.id}><Link to={'/app/' + tenantId + '/conversations/' + channel + '/' + item.id} className={'block px-4 py-4 transition hover:bg-white/[0.035] ' + (item.id === conversationId ? 'bg-signal/10 ' : '') + (item.human_attention_state === 'REQUESTED' ? 'bg-red-500/10 ring-1 ring-inset ring-red-400/30' : '')}><div className="flex justify-between gap-2"><div className="min-w-0"><p className="truncate text-sm font-medium text-ink">{displayConversationCustomerIdentifier(item.customer_external_id || item.external_conversation_id)}</p><p className="mt-1 truncate text-xs text-stone-400">{item.last_message_preview || item.channel_display_name || 'No message preview'}</p></div><time className="shrink-0 text-[11px] text-stone-500">{formatDateTime(item.last_activity_at || item.created_at)}</time></div><div className="mt-3 flex gap-1.5"><span className="rounded bg-white/[0.05] px-1.5 py-0.5 text-[10px] text-stone-300">{handlingLabel(item.handling_mode)}</span>{item.human_attention_state === 'REQUESTED' && <span className="rounded border border-red-400/40 bg-red-500/15 px-1.5 py-0.5 text-[10px] font-bold tracking-[0.1em] text-red-200">LIVE SUPPORT</span>}</div></Link></li>)}</ul>}</div>
        ...
        {!conversationId ? <EmptyState title="Select a conversation" description="Choose an inbox item to inspect its real message history." icon={<MessageSquareText size={22} />} /> : conversationQuery.isLoading || messagesQuery.isLoading ? <div className="space-y-4 p-5"><SkeletonBlock className="h-20" /><SkeletonBlock className="h-80" /></div> : conversationQuery.isError || messagesQuery.isError ? <QueryErrorState error={conversationQuery.error ?? messagesQuery.error!} onRetry={() => { void conversationQuery.refetch(); void messagesQuery.refetch(); }} resource="conversation" /> : conversation ? <>
          ...
          <div ref={messageListRef} onScroll={(event) => { const element = event.currentTarget; atMessageBottomRef.current = element.scrollHeight - element.scrollTop - element.clientHeight < 72; if (element.scrollTop < 64) void loadOlderMessages(); }} className="flex-1 space-y-4 overflow-y-auto bg-black/10 p-5">{messagesQuery.hasNextPage && <div className="flex justify-center"><button type="button" onClick={() => void loadOlderMessages()} disabled={messagesQuery.isFetchingNextPage} className="rounded border border-white/10 px-3 py-1.5 text-xs text-stone-300 hover:bg-white/[0.04] disabled:opacity-50">{messagesQuery.isFetchingNextPage ? 'Loading earlier messages…' : 'Load earlier messages'}</button></div>}{messages.map((message) => {
            ...
            const bubbleTone = isCustomer
              ? 'border-white/10 bg-[#1a1a1e] text-stone-100'
              : isAgentMessage
                ? 'border-gold/30 bg-gold/10 text-stone-50'
                : 'border-red-400/25 bg-red-500/15 text-stone-50';
      ...
      <aside className="panel hidden min-h-0 overflow-y-auto p-4 xl:block">{conversation ? <div className="space-y-3">
        <section className="rounded-xl border border-white/[0.08] bg-white/[0.025] p-4"><p className="text-[10px] font-semibold tracking-[0.16em] text-stone-500">CONTACT INFORMATION</p><p className="mt-3 break-all text-sm font-semibold text-ink">{conversation.customer_external_id ? displayConversationCustomerIdentifier(conversation.customer_external_id) : 'Customer identifier unavailable'}</p><dl className="mt-3 space-y-2 text-xs"><div className="flex justify-between gap-3"><dt className="text-stone-500">Channel</dt><dd className="text-right text-stone-200">{conversation.channel_display_name || channelContext.label}</dd></div><div className="flex justify-between gap-3"><dt className="text-stone-500">Last activity</dt><dd className="text-right text-stone-200">{formatDateTime(conversation.last_activity_at)}</dd></div></dl></section>
        ...
        <section className="rounded-xl border border-white/[0.08] bg-white/[0.025] p-4"><p className="text-[10px] font-semibold tracking-[0.16em] text-stone-500">CONVERSATION ANALYTICS</p><div className="mt-3 grid grid-cols-2 gap-2 text-xs"><div className="rounded-lg bg-black/20 p-2.5"><p className="text-stone-500">Messages</p><p className="mt-1 text-base font-semibold text-ink">{messageMetrics.total}</p></div><div className="rounded-lg bg-black/20 p-2.5"><p className="text-stone-500">Attachments</p><p className="mt-1 text-base font-semibold text-ink">{messageMetrics.attachments}</p></div><div className="rounded-lg bg-black/20 p-2.5"><p className="text-stone-500">Customer</p><p className="mt-1 text-base font-semibold text-ink">{messageMetrics.customer}</p></div><div className="rounded-lg bg-black/20 p-2.5"><p className="text-stone-500">AI / Agent</p><p className="mt-1 text-base font-semibold text-ink">{messageMetrics.assistant + messageMetrics.agent}</p></div></div><div className="mt-3 border-t border-white/[0.06] pt-3 text-xs"><p className="font-medium text-ink">{conversation.human_attention_state === 'REQUESTED' ? 'LIVE SUPPORT WAITING' : handlingLabel(conversation.handling_mode)}</p><p className="mt-1 text-stone-500">{conversation.assigned_agent_email ? 'Assigned operator: ' + conversation.assigned_agent_email : 'No operator assigned'}</p></div></section>
