...
import { canTakeOverConversation, canUseHumanReplyComposer, clearSentAgentDraft, deliveryTickPresentation, displayConversationCustomerIdentifier, formatInstagramCustomerDisplay, isInlinePreviewableAttachment, isVoiceResource, resourceDisplayName, senderLabel, voiceResourceDisplayLabel } from './conversation-utils';
...
import { HighlightedText, SafeRichMessage } from './safe-rich-message';
...
import { messageMatchesSearch, nextSearchMatchIndex, normalizeSearchTokens } from './conversation-search-utils';
...
export function ConversationsPage() {
  ...
  const humanTypingStopTimerRef = useRef<number | null>(null);
  const humanTypingLastPublishedRef = useRef(0);
  ...
  useEffect(() => {
    if (!messageSearchOpen && !actionsOpen) return;
    const dismiss = (event: MouseEvent) => {
      if (conversationHeaderRef.current?.contains(event.target as Node)) return;
  ...
  const operation = useMutation({
    mutationFn: (action: string) => {
      ...
      if (action === 'archive') return tenantApi.archiveConversation(tenantId, conversationId);
      if (action === 'unarchive') return tenantApi.unarchiveConversation(tenantId, conversationId);
      return tenantApi.closeConversation(tenantId, conversationId);
    },

    onSuccess: async (_data, action) => { await refresh(action); },
  });
  const aiOverride = useMutation({
    mutationFn: (override: import('../../types/api').AiBehaviorOverride) => {
      if (!conversationId) throw new Error('Conversation not selected');
  ...
  const messageSearchTokens = normalizeSearchTokens(messageSearch);
  const listSearchTokens = normalizeSearchTokens(debouncedSearch);
  const messageSearchMatches = messageSearchTokens.length ? messages.filter((message) => messageMatchesSearch({
    content: message.content,
    resourceNames: message.resources.map((resource) => resourceDisplayName(resource)),
  }, messageSearchTokens)) : [];
  ...
  const setHumanTyping = (active: boolean) => {
    if (!conversationId || !canSend) return;
    if (humanTypingStopTimerRef.current !== null) {
      window.clearTimeout(humanTypingStopTimerRef.current);
      humanTypingStopTimerRef.current = null;
    }
    if (active) {
      const now = Date.now();
      if (now - humanTypingLastPublishedRef.current >= 800) {
        humanTypingLastPublishedRef.current = now;
        void tenantApi.setHumanTyping(tenantId, conversationId, true).catch(() => {});
      }
      humanTypingStopTimerRef.current = window.setTimeout(() => setHumanTyping(false), 1800);
      return;
    }
    void tenantApi.setHumanTyping(tenantId, conversationId, false).catch(() => {});
  };
  useEffect(() => () => {
    if (humanTypingStopTimerRef.current !== null) window.clearTimeout(humanTypingStopTimerRef.current);
  }, [conversationId]);
    ...
    <div className="grid h-[calc(100vh-14rem)] min-h-[38rem] gap-2 xl:grid-cols-[minmax(17rem,21rem)_minmax(0,1fr)_minmax(18rem,22rem)]">
      ...
      <section className={'dashboard-card flex min-h-[42rem] flex-col overflow-hidden rounded-lg ' + (!conversationId ? 'hidden xl:flex' : '')}>
        {!conversationId ? <EmptyState title="Select a conversation" description="Choose an inbox item to inspect its real message history." icon={<MessageSquareText size={22} />} /> : conversationQuery.isLoading || messagesQuery.isLoading ? <div className="space-y-4 p-5"><SkeletonBlock className="h-20" /><SkeletonBlock className="h-80" /></div> : conversationQuery.isError || messagesQuery.isError ? <QueryErrorState error={conversationQuery.error ?? messagesQuery.error!} onRetry={() => { void conversationQuery.refetch(); void messagesQuery.refetch(); }} resource="conversation" /> : conversation ? <>
          <header ref={conversationHeaderRef} className="relative border-b border-line bg-elevated/35 px-5 py-3.5"><div className="flex flex-wrap items-center justify-between gap-3"><div><Link to={'/app/' + tenantId + '/conversations'} className="text-xs text-stone-400 xl:hidden">← Inbox</Link><p className="mt-1 text-base font-semibold text-ink">{conversation.channel_type === 'INSTAGRAM' ? formatInstagramCustomerDisplay(conversation.contact_display_name, null, conversation.customer_external_id) : (conversation.contact_display_name || displayConversationCustomerIdentifier(conversation.customer_external_id, conversation.channel_type))}</p><p className="mt-1 text-xs text-stone-400">{conversation.channel_type === 'INSTAGRAM' ? (conversation.contact_phone ? `${conversation.contact_phone} · ` : '') + (conversation.channel_display_name || channelContext.label) + ' · ' + handlingLabel(conversation.handling_mode) : (conversation.contact_phone || displayConversationCustomerIdentifier(conversation.customer_external_id, conversation.channel_type)) + ' · ' + (conversation.channel_display_name || channelContext.label) + ' · ' + handlingLabel(conversation.handling_mode)}</p>
</div><div className="relative flex items-center gap-2"><button type="button" onClick={() => { setActionsOpen(false); setMessageSearchOpen((open) => !open); }} className={'grid h-9 w-9 place-items-center rounded-lg border text-stone-200 transition hover:bg-white/[.06] ' + (messageSearchOpen ? (channelContext.type === 'WHATSAPP' ? 'border-emerald-300/60 bg-emerald-500/15' : 'border-red-300/60 bg-red-500/15') : 'border-line bg-black/10')} aria-label="Search in conversation"><Search size={16} /></button>{canTakeOver && <button type="button" onClick={() => operation.mutate('takeover')} className="inline-flex items-center gap-1.5 rounded-lg border border-gold/35 bg-gold/10 px-3 py-2 text-xs font-semibold text-gold shadow-[0_0_20px_rgba(224,169,79,.1)] transition hover:bg-gold/20"><Headphones size={15} />Take Over</button>}{canReturn && <button type="button" onClick={() => operation.mutate('return')} className="inline-flex items-center gap-1.5 rounded-lg border border-emerald-400/30 bg-emerald-400/10 px-3 py-2 text-xs font-semibold text-emerald-200 transition hover:bg-emerald-400/20"><Bot size={15} />Return to AI</button>}{isAdmin && conversation.handling_mode === 'AI' && <button type="button" onClick={() => operation.mutate('pause')} className="inline-flex items-center gap-1.5 rounded-lg border border-line bg-black/10 px-3 py-2 text-xs font-medium text-stone-200 hover:bg-white/[.06]"><Pause size={14} />Pause AI</button>}<button type="button" aria-label="Conversation actions" aria-expanded={actionsOpen} onClick={() => { setMessageSearchOpen(false); setMessageSearch(''); setActionsOpen((open) => !open); }} className="grid h-9 w-9 place-items-center rounded-lg border border-line bg-black/10 text-stone-300 transition hover:bg-white/[.06] hover:text-white"><MoreHorizontal size={18} /></button>{actionsOpen && <div className="absolute right-0 top-11 z-30 w-44 overflow-hidden rounded-xl border border-line bg-[#101a27] p-1.5 shadow-2xl"><p className="px-2.5 py-1 text-[9px] font-semibold uppercase tracking-[.16em] text-stone-500">Conversation actions</p>{isAdmin && conversation.handling_mode === 'PAUSED' && <button type="button" onClick={() => { setActionsOpen(false); operation.mutate('resume'); }} className="flex w-full items-center gap-2 rounded-lg px-2.5 py-2 text-left text-xs text-stone-200 hover:bg-white/[.06]"><Pause size={14} />Resume AI</button>}{isAdmin && conversation.status === 'open' && <button type="button" onClick={() => { setActionsOpen(false); operation.mutate('close'); }} className="flex w-full items-center gap-2 rounded-lg px-2.5 py-2 text-left text-xs text-red-300 hover:bg-red-500/10"><CircleX size={14} />Close conversation</button>}{isAdmin && <button type="button" onClick={() => { setActionsOpen(false); operation.mutate(conversation.status === 'archived' ? 'unarchive' : 'archive'); }} className="flex w-full items-center gap-2 rounded-lg px-2.5 py-2 text-left text-xs text-stone-200 hover:bg-white/[.06]"><CircleX size={14} />{conversation.status === 'archived' ? 'Restore conversation' : 'Archive conversation'}</button>}</div>}</div></div>{messageSearchOpen && <div className={'mt-3 flex items-center gap-2 rounded-xl border bg-black/15 p-2 ' + (channelContext.type === 'WHATSAPP' ? 'border-emerald-300/45' : 'border-red-300/45')}><Search size={15} className={channelContext.type === 'WHATSAPP' ? 'text-emerald-300' : 'text-red-300'} /><input autoFocus value={messageSearch} onChange={(event) => setMessageSearch(event.target.value)} onKeyDown={(event) => { if (event.key === 'Escape') { setMessageSearchOpen(false); setMessageSearch(''); } if ((event.key === 'ArrowDown' || (event.key === 'Enter' && !event.shiftKey)) && messageSearchMatches.length) { event.preventDefault(); setMessageSearchIndex((index) => nextSearchMatchIndex(index, messageSearchMatches.length, 'next')); } if ((event.key === 'ArrowUp' || (event.key === 'Enter' && event.shiftKey)) && messageSearchMatches.length) { event.preventDefault(); setMessageSearchIndex((index) => nextSearchMatchIndex(index, messageSearchMatches.length, 'previous')); } }} className="min-w-0 flex-1 bg-transparent px-1 py-1 text-xs text-ink outline-none placeholder:text-stone-500" placeholder="Search in conversation" aria-label="Search in conversation" /><span className="shrink-0 text-[11px] text-stone-400">{messageSearch ? (messageSearchMatches.length ? (messageSearchIndex + 1) + ' of ' + messageSearchMatches.length : 'No matches') : 'Search messages'}</span><button type="button" onClick={() => messageSearchMatches.length && setMessageSearchIndex((index) => nextSearchMatchIndex(index, messageSearchMatches.length, 'previous'))} disabled={!messageSearchMatches.length} className="rounded px-1.5 py-1 text-stone-300 hover:bg-white/[.06] disabled:opacity-40" aria-label="Previous match">↑</button><button type="button" onClick={() => messageSearchMatches.length && setMessageSearchIndex((index) => nextSearchMatchIndex(index, messageSearchMatches.length, 'next'))} disabled={!messageSearchMatches.length} className="rounded px-1.5 py-1 text-stone-300 hover:bg-white/[.06] disabled:opacity-40" aria-label="Next match">↓</button><button type="button" onClick={() => { setMessageSearchOpen(false); setMessageSearch(''); }} className="grid h-7 w-7 place-items-center rounded text-stone-300 hover:bg-white/[.06]" aria-label="Close message search"><X size={15} /></button></div>}{operation.error instanceof Error && <p role="alert" className="mt-3 text-xs text-red-300">{operation.error.message}</p>}</header>
          ...
          <div ref={messageListRef} onScroll={(event) => { const element = event.currentTarget; atMessageBottomRef.current = element.scrollHeight - element.scrollTop - element.clientHeight < 72; if (atMessageBottomRef.current) { setNewMessagesWaiting(false); setFirstNewMessageId(null); } if (element.scrollTop < 64) void loadOlderMessages(); }} className={"subtle-scrollbar flex-1 space-y-3 overflow-y-auto p-4 sm:p-5 [background-image:radial-gradient(circle_at_20%_20%,rgba(80,108,124,.11)_1px,transparent_1px),radial-gradient(circle_at_80%_75%,rgba(65,112,95,.10)_1px,transparent_1px)] [background-size:22px_22px] " + workspaceVisualIdentity(channelContext.type).canvas}>{messagesQuery.hasNextPage && <div className="flex justify-center"><button type="button" onClick={() => void loadOlderMessages()} disabled={messagesQuery.isFetchingNextPage} className="rounded border border-white/10 px-3 py-1.5 text-xs text-stone-300 hover:bg-white/[0.04] disabled:opacity-50">{messagesQuery.isFetchingNextPage ? 'Loading earlier messages…' : 'Load earlier messages'}</button></div>}{messages.map((message) => {
            ...
            const contactIdentity = conversation.contact_display_name || conversation.contact_phone || displayConversationCustomerIdentifier(conversation.customer_external_id, conversation.channel_type);
      ...
      <aside className="dashboard-card hidden min-h-0 overflow-y-auto rounded-lg bg-[#09111B] p-3 xl:block">{conversation ? <div className="space-y-3">
        <section className="rounded-lg border border-line bg-elevated/45 p-4"><p className="text-[10px] font-semibold tracking-[0.16em] text-stone-500">CONTACT INFORMATION</p><p className="mt-3 break-all text-sm font-semibold text-ink">{conversation.channel_type === 'INSTAGRAM' ? formatInstagramCustomerDisplay(conversation.contact_display_name, null, conversation.customer_external_id) : (conversation.contact_display_name || (conversation.customer_external_id ? displayConversationCustomerIdentifier(conversation.customer_external_id, conversation.channel_type) : 'Customer identifier unavailable'))}</p><dl className="mt-3 space-y-2 text-xs"><div className="flex justify-between gap-3"><dt className="text-stone-500">Phone</dt><dd className="text-right text-stone-200">{conversation.contact_phone || '—'}</dd></div>
<div className="flex justify-between gap-3"><dt className="text-stone-500">Language</dt><dd className="text-right text-stone-200">{({ tr: 'Turkish', en: 'English', ar: 'Arabic', es: 'Spanish', fr: 'French', de: 'German' } as Record<string, string>)[conversation.communication_language || conversation.contact_language || ''] || '—'}</dd></div><div className="flex justify-between gap-3"><dt className="text-stone-500">Country</dt><dd className="text-right text-stone-200">{conversation.contact_country || '—'}</dd></div><div className="flex justify-between gap-3"><dt className="text-stone-500">Channel</dt><dd className="text-right text-stone-200">{conversation.channel_display_name || channelContext.label}</dd></div><div className="flex justify-between gap-3"><dt className="text-stone-500">Last activity</dt><dd className="text-right text-stone-200">{formatDateTime(conversation.last_activity_at)}</dd></div></dl></section>
