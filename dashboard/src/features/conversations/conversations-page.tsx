...
import { canTakeOverConversation, canUseHumanReplyComposer, clearSentAgentDraft, isInlinePreviewableAttachment, senderLabel } from './conversation-utils';
  ...
  return <div className="space-y-4">
    ...
    <div className="grid h-[calc(100vh-12.5rem)] min-h-[40rem] gap-3 xl:grid-cols-[minmax(17rem,22rem)_minmax(0,1fr)_minmax(18rem,23rem)]">
      ...
      <aside className="panel hidden min-h-0 overflow-y-auto p-4 xl:block">{conversation ? <div className="space-y-3">
        <section className="rounded-xl border border-white/[0.08] bg-white/[0.025] p-4"><p className="text-[10px] font-semibold tracking-[0.16em] text-stone-500">CONTACT INFORMATION</p><p className="mt-3 break-all text-sm font-semibold text-ink">{conversation.customer_external_id || 'Customer identifier unavailable'}</p><dl className="mt-3 space-y-2 text-xs"><div className="flex justify-between gap-3"><dt className="text-stone-500">Channel</dt><dd className="text-right text-stone-200">{conversation.channel_display_name || channelContext.label}</dd></div><div className="flex justify-between gap-3"><dt className="text-stone-500">Last activity</dt><dd className="text-right text-stone-200">{formatDateTime(conversation.last_activity_at)}</dd></div></dl></section>
