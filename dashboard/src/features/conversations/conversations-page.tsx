...
export function ConversationsPage() {
  ...
  const liveState = useTenantConversationLiveEvents(tenantId);
  useEffect(() => {
    setMessageOffset(0);
  }, [conversationId]);
  ...
  const canSend = Boolean(conversation && conversation.status === 'open' && conversation.channel_type === 'SAMCHEGUIDE' && conversation.handling_mode === 'HUMAN' && (isAdmin || (isAgent && isOwn)));
    ...
    <div className="grid min-h-[42rem] gap-4 xl:grid-cols-[19rem_minmax(0,1fr)_18rem]">
      ...
      <section className={'panel flex min-h-[42rem] flex-col overflow-hidden ' + (!conversationId ? 'hidden xl:flex' : '')}>
        {!conversationId ? <EmptyState title="Select a conversation" description="Choose an inbox item to inspect its real message history." icon={<MessageSquareText size={22} />} /> : conversationQuery.isLoading || messagesQuery.isLoading ? <div className="space-y-4 p-5"><SkeletonBlock className="h-20" /><SkeletonBlock className="h-80" /></div> : conversationQuery.isError || messagesQuery.isError ? <QueryErrorState error={conversationQuery.error ?? messagesQuery.error!} onRetry={() => { void conversationQuery.refetch(); void messagesQuery.refetch(); }} resource="conversation" /> : conversation ? <>
          ...
          <div className="flex-1 space-y-4 overflow-y-auto bg-black/10 p-5">{messages.map((message) => <article key={message.id} className={'max-w-[88%] rounded-xl border px-4 py-3 ' + senderTone(message.sender_type)}><div className="flex justify-between gap-3"><p className="text-[10px] font-semibold uppercase tracking-[0.12em] opacity-70">{senderLabel(message.sender_type)}{message.actor_email ? ' · ' + message.actor_email : ''}</p><time className="text-[10px] opacity-60">{formatDateTime(message.created_at)}</time></div><p className="mt-2 whitespace-pre-wrap text-sm leading-6">{message.content}</p></article>)}{messages.length === 0 && <EmptyState title="No messages yet" description="Messages will appear as the channel receives them." />}{messages.length > 0 && <div className="flex justify-between pt-2 text-xs text-stone-400"><button type="button" onClick={() => setMessageOffset((value) => Math.max(0, value - messagePageSize))} disabled={messageOffset === 0} className="disabled:opacity-40"><ChevronLeft className="inline" size={14} /> Previous messages</button><button type="button" onClick={() => setMessageOffset((value) => value + messagePageSize)} disabled={messages.length < messagePageSize} className="disabled:opacity-40">Next messages <ChevronRight className="inline" size={14} /></button></div>}</div>
          <footer className="border-t border-line bg-black/10 px-5 py-4">{canSend ? <form onSubmit={(event) => { event.preventDefault(); if (content.trim()) send.mutate(); }}><label className="sr-only" htmlFor="agent-message">Reply as human agent</label><textarea id="agent-message" value={content} onChange={(event) => setContent(event.target.value)} className="field min-h-24 w-full resize-y" placeholder="Write a response to the customer…" maxLength={8000} /><div className="mt-3 flex justify-between gap-3"><p className="text-xs text-stone-500">Delivered through the Samcheguide conversation feed.</p><button type="submit" disabled={!content.trim() || send.isPending} className="button-primary"><Send size={15} />Send</button></div>{send.error instanceof Error && <p role="alert" className="mt-2 text-xs text-red-300">{send.error.message}</p>}</form> : <p className="text-xs text-stone-400">{conversation.channel_type === 'WHATSAPP' ? 'Human delivery is not configured for this tenant-safe channel yet.' : conversation.handling_mode === 'HUMAN' ? 'Only the assigned operator can reply.' : 'Take over to enable a supported human reply.'}</p>}</footer>
