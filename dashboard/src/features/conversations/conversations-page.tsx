...
export function ConversationsPage() {
  ...
  return <div className="mx-auto max-w-[1600px] space-y-3">
    <header className="flex items-center justify-between gap-4 px-1"><div><p className="eyebrow">Live customer inbox</p><h1 className="mt-1 text-xl font-semibold tracking-tight text-ink">Conversations <span className="text-stone-500">/</span> {channelContext.label}</h1></div><LiveState state={liveState} /></header>
    <div className="grid h-[calc(100vh-9.75rem)] min-h-[38rem] gap-2 xl:grid-cols-[minmax(17rem,21rem)_minmax(0,1fr)_minmax(18rem,22rem)]">
      ...
      <section className={'dashboard-card flex min-h-[42rem] flex-col overflow-hidden rounded-lg ' + (!conversationId ? 'hidden xl:flex' : '')}>
        {!conversationId ? <EmptyState title="Select a conversation" description="Choose an inbox item to inspect its real message history." icon={<MessageSquareText size={22} />} /> : conversationQuery.isLoading || messagesQuery.isLoading ? <div className="space-y-4 p-5"><SkeletonBlock className="h-20" /><SkeletonBlock className="h-80" /></div> : conversationQuery.isError || messagesQuery.isError ? <QueryErrorState error={conversationQuery.error ?? messagesQuery.error!} onRetry={() => { void conversationQuery.refetch(); void messagesQuery.refetch(); }} resource="conversation" /> : conversation ? <>
          ...
          {attachmentError && <p role="alert" className="mx-5 mt-3 text-xs text-red-300">{attachmentError}</p>}<footer className="border-t border-line bg-elevated/35 px-4 py-3.5">{canSend ? <form onSubmit={(event: FormEvent<HTMLFormElement>) => { event.preventDefault(); if (content.trim()) send.mutate(content.trim()); }}><label className="sr-only" htmlFor="agent-message">Reply as human agent</label><textarea id="agent-message" value={content} onChange={(event) => setContent(event.target.value)} className="field min-h-20 w-full resize-y rounded-xl bg-[#08111C] px-3 py-2.5" placeholder="Write a response to the customer…" maxLength={8000} /><div className="mt-3 flex justify-between gap-3"><p className="text-xs text-stone-500">{conversation.channel_type === 'WHATSAPP' ? 'Sent through the configured WhatsApp channel.' : 'Sent to the Samcheguide conversation feed.'}</p><button type="submit" disabled={!content.trim() || send.isPending} className="button-primary"><Send size={15} />Send</button></div>{send.error instanceof Error && <p role="alert" className="mt-2 text-xs text-red-300">{send.error.message}</p>}</form> : <p className="text-xs text-stone-400">{conversation.handling_mode === 'HUMAN' ? 'Only the assigned operator can reply.' : 'Take over to enable a supported human reply.'}</p>}</footer>
