...
export function ConversationsPage() {
  ...
  return <div className="mx-auto max-w-[1600px] space-y-3">
    {channelContext.type === 'WHATSAPP' ? <header className="relative overflow-hidden rounded-xl border border-emerald-500/15 bg-[#08131a] px-5 py-4 text-center shadow-panel"><div className="pointer-events-none absolute inset-0 opacity-30 [background-image:radial-gradient(circle_at_15%_20%,#21c675_1px,transparent_1px),radial-gradient(circle_at_85%_20%,#21c675_1px,transparent_1px)] [background-size:18px_18px]" /><div className="relative"><h1 className="text-2xl font-semibold tracking-tight text-ink"><span className="text-[#20c977]">WhatsApp</span> AI Assistant</h1><p className="mt-1 text-sm text-stone-300">Automate conversations. Answer instantly. Close more deals.</p><div className="mt-3 flex flex-wrap justify-center gap-x-5 gap-y-1 text-xs text-stone-300"><span>AI-Powered Replies</span><span>Document Reading</span><span>Multi-language</span></div></div></header> : <header className="flex items-center justify-between gap-4 px-1"><div><p className="eyebrow">Live customer inbox</p><h1 className="mt-1 text-xl font-semibold tracking-tight text-ink">Conversations <span className="text-stone-500">/</span> {channelContext.label}</h1></div><LiveState state={liveState} /></header>}
    <div className="grid h-[calc(100vh-14rem)] min-h-[38rem] gap-2 xl:grid-cols-[minmax(17rem,21rem)_minmax(0,1fr)_minmax(18rem,22rem)]">
      ...
      <section className={'dashboard-card flex min-h-[42rem] flex-col overflow-hidden rounded-lg ' + (!conversationId ? 'hidden xl:flex' : '')}>
        {!conversationId ? <EmptyState title="Select a conversation" description="Choose an inbox item to inspect its real message history." icon={<MessageSquareText size={22} />} /> : conversationQuery.isLoading || messagesQuery.isLoading ? <div className="space-y-4 p-5"><SkeletonBlock className="h-20" /><SkeletonBlock className="h-80" /></div> : conversationQuery.isError || messagesQuery.isError ? <QueryErrorState error={conversationQuery.error ?? messagesQuery.error!} onRetry={() => { void conversationQuery.refetch(); void messagesQuery.refetch(); }} resource="conversation" /> : conversation ? <>
          ...
          <div ref={messageListRef} onScroll={(event) => { const element = event.currentTarget; atMessageBottomRef.current = element.scrollHeight - element.scrollTop - element.clientHeight < 72; if (element.scrollTop < 64) void loadOlderMessages(); }} className="flex-1 space-y-3 overflow-y-auto bg-[#07101A] p-4 sm:p-5 [background-image:radial-gradient(circle_at_20%_20%,rgba(65,112,95,.16)_1px,transparent_1px),radial-gradient(circle_at_80%_75%,rgba(56,105,90,.12)_1px,transparent_1px)] [background-size:22px_22px]">{messagesQuery.hasNextPage && <div className="flex justify-center"><button type="button" onClick={() => void loadOlderMessages()} disabled={messagesQuery.isFetchingNextPage} className="rounded border border-white/10 px-3 py-1.5 text-xs text-stone-300 hover:bg-white/[0.04] disabled:opacity-50">{messagesQuery.isFetchingNextPage ? 'Loading earlier messages…' : 'Load earlier messages'}</button></div>}{messages.map((message) => {
            const isCustomer = message.sender_type === 'CUSTOMER';
            const isAgentMessage = message.sender_type === 'AGENT';
            const bubbleTone = isCustomer
              ? 'border-black/10 bg-[#f7f7f3] text-[#142018]'
              : isAgentMessage
                ? 'border-emerald-300/20 bg-[#0d7a50] text-white'
                : 'border-emerald-300/20 bg-[#13785a] text-white';
