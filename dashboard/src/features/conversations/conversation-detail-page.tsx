...
import { senderLabel, senderTone, formatInstagramCustomerDisplay, displayConversationCustomerIdentifier } from './conversation-utils';
...
export function ConversationDetailPage({ tenantId, conversationId, mobileBackPath }: { tenantId: string; conversationId: string; mobileBackPath?: string }) {
  ...
  {conversation.channel_type === 'INSTAGRAM'
    ? formatInstagramCustomerDisplay(conversation.contact_display_name, null, conversation.customer_external_id)
    : (conversation.contact_display_name || conversation.customer_external_id || conversation.external_conversation_id || 'Customer conversation')}
</p>
