...
export const tenantApi = {
  listPlans: () => apiClient.get<{ plans: import('../../types/api').PlatformPlan[] }>('/api/v1/tenants/plans').then((value) => value.plans),
  getTenantPlan: (tenantId: string) => apiClient.get<{ plan: { plan_code: string; display_name: string; customer_subtitle: string; rank: number; billing_cycle?: 'MONTHLY' | 'ANNUAL'; monthly_price_aed?: number; annual_price_aed?: number; setup_fee_aed?: number; currency?: string; pending_request?: { requested_plan_code: string } | null } }>(`${tenantRoot(tenantId)}/plan`).then((value) => value.plan),
  getTenantSubscription: (tenantId: string) => apiClient.get<{ subscription: import('../../types/api').TenantSubscriptionInfo; capabilities: Record<string, import('../../types/api').TenantCapabilityState>; limits: Record<string, import('../../types/api').TenantUsageLimitState>; locked_capabilities: import('../../types/api').LockedCapability[] }>(`${tenantRoot(tenantId)}/subscription`),
  getTenantEntitlements: (tenantId: string) => apiClient.get<import('../../types/api').EffectiveTenantEntitlements>(`${tenantRoot(tenantId)}/entitlements`),
  changeTenantPlanAsOwner: (tenantId: string, planCode: 'STARTER' | 'GROWTH' | 'BUSINESS' | 'ENTERPRISE', billingCycle: 'MONTHLY' | 'ANNUAL' = 'MONTHLY') => apiClient.put<{ plan: { plan_code: string; display_name: string; customer_subtitle: string; rank: number; billing_cycle?: string } }>(`${tenantRoot(tenantId)}/plan`, { plan_code: planCode, billing_cycle: billingCycle }).then((value) => value.plan),
  grantEntitlementOverride: (tenantId: string, capabilityKey: string, effect: 'GRANT' | 'DENY' = 'GRANT', reason?: string) => apiClient.post<{ override: unknown }>(`${tenantRoot(tenantId)}/overrides`, { capability_key: capabilityKey, effect, reason }).then((v) => v.override),
  revokeEntitlementOverride: (tenantId: string, capabilityKey: string) => apiClient.delete<{ success: boolean; revoked: boolean }>(`${tenantRoot(tenantId)}/overrides/${capabilityKey}`),
  updateUsageAllocation: (tenantId: string, metricKey: string, allocatedLimit: number) => apiClient.put<{ allocation: unknown }>(`${tenantRoot(tenantId)}/limits`, { metric_key: metricKey, allocated_limit: allocatedLimit }).then((v) => v.allocation),
  getEntitlementsAuditLog: (tenantId: string) => apiClient.get<{ audit: import('../../types/api').EntitlementAuditLogEntry[] }>(`${tenantRoot(tenantId)}/entitlements/audit`).then((v) => v.audit),
  ...
  listGuideExperiences: (tenantId: string, assistantId: string) => apiClient.get<{ versions: import('../../types/api').GuideExperienceVersion[] }>(`${tenantRoot(tenantId)}/guide-experiences/assistants/${assistantId}`).then((value) => value.versions),
  createGuideExperienceDraft: (tenantId: string, assistantId: string, experience: import('../../types/api').GuideExperienceData) => apiClient.post<{ version: import('../../types/api').GuideExperienceVersion }>(`${tenantRoot(tenantId)}/guide-experiences/assistants/${assistantId}/drafts`, { experience }).then((value) => value.version),
  updateGuideExperienceDraft: (tenantId: string, assistantId: string, versionId: string, experience: import('../../types/api').GuideExperienceData) => apiClient.put<{ version: import('../../types/api').GuideExperienceVersion }>(`${tenantRoot(tenantId)}/guide-experiences/assistants/${assistantId}/drafts/${versionId}`, { experience }).then((value) => value.version),
  createRecommendedGuideExperienceDraft: (tenantId: string, assistantId: string) => apiClient.post<{ version: import('../../types/api').GuideExperienceVersion; recommendation: { classification: { sector: string; capabilities: string[]; source: string }; facts_used: { active_profile: boolean; active_configuration: boolean } } }>(`${tenantRoot(tenantId)}/guide-experiences/assistants/${assistantId}/recommendations/draft`, {}),
  recommendGuideTheme: (tenantId: string, assistantId: string, candidates: string[]) => apiClient.post<{ theme: import('../../types/api').GuideExperienceData['theme'] }>(`${tenantRoot(tenantId)}/guide-experiences/assistants/${assistantId}/theme-recommendation`, { candidates }).then((value) => value.theme),
  publishGuideExperience: (tenantId: string, assistantId: string, versionId: string) => apiClient.post<{ version: import('../../types/api').GuideExperienceVersion }>(`${tenantRoot(tenantId)}/guide-experiences/assistants/${assistantId}/drafts/${versionId}/publish`, {}).then((value) => value.version),
  previewGuideExperience: (tenantId: string, assistantId: string, versionId: string) => apiClient.post<{ preview_path: string; hostname: string; expires_in_seconds: number }>(`${tenantRoot(tenantId)}/guide-experiences/assistants/${assistantId}/drafts/${versionId}/preview`, {}),
  rollbackGuideExperience: (tenantId: string, assistantId: string, versionId: string) => apiClient.post<{ version: import('../../types/api').GuideExperienceVersion }>(`${tenantRoot(tenantId)}/guide-experiences/assistants/${assistantId}/versions/${versionId}/rollback`, { confirmed: true }).then((value) => value.version),
  uploadGuideExperienceAsset: (tenantId: string, assistantId: string, kind: 'LOGO' | 'AVATAR', file: File) => { const form = new FormData(); form.append('kind', kind); form.append('file', file); return apiClient.postForm<{ asset: { id: string; public_url: string; mime_type: string; size_bytes: number } }>(`${tenantRoot(tenantId)}/guide-experiences/assistants/${assistantId}/assets`, form).then((value) => value.asset); },
  listGuideDomains: (tenantId: string, assistantId: string) => apiClient.get<{ domains: import('../../types/api').GuideDomain[]; managed_domain_suffix?: string }>(`${tenantRoot(tenantId)}/guide-experiences/assistants/${assistantId}/domains`).then((value) => Object.assign(value.domains, { managed_domain_suffix: value.managed_domain_suffix }) as import('../../types/api').GuideDomain[] & { managed_domain_suffix?: string }),
  getGuidePublicationDiagnostics: (tenantId: string, assistantId: string) => apiClient.get<{ consistency: string; current_published: { id: string; version: number; status: string } | null; public_bootstrap_version: number | null; versions: Array<{ id: string; version: number; status: string; created_at?: string; published_at?: string | null; updated_at?: string }> }>(`${tenantRoot(tenantId)}/guide-experiences/assistants/${assistantId}/publication-diagnostics`),
  createGuideDomain: (tenantId: string, assistantId: string, body: { hostname?: string; slug?: string; domain_mode: 'MANAGED' | 'CUSTOM'; channel_id: string }) => apiClient.post<{ domain: import('../../types/api').GuideDomain }>(`${tenantRoot(tenantId)}/guide-experiences/assistants/${assistantId}/domains`, body).then((value) => value.domain),
  verifyGuideDomain: (tenantId: string, assistantId: string, domainId: string) => apiClient.post<{ domain: import('../../types/api').GuideDomain }>(`${tenantRoot(tenantId)}/guide-experiences/assistants/${assistantId}/domains/${domainId}/verify`, {}).then((value) => value.domain),
  archiveGuideDomain: (tenantId: string, assistantId: string, domainId: string) => apiClient.delete<{ domain: import('../../types/api').GuideDomain }>(`${tenantRoot(tenantId)}/guide-experiences/assistants/${assistantId}/domains/${domainId}`).then((value) => value.domain),
  ...
  previewWebChatTheme: (tenantId: string, body: {
    ...
    launcher_foreground?: string | null;
    launcher_text?: string | null;
    launcher_border_color?: string | null;
    launcher_border?: string | null;
    launcher_glow_color?: string | null;
    launcher_glow?: string | null;
    launcher_logo_background?: string | null;
  ...
  getBusinessProfileGenerationJob: (tenantId: string, jobId: string) => apiClient.get<{ job: BusinessProfileGenerationJob }>(`${tenantRoot(tenantId)}/knowledge-intelligence/profiles/generation-jobs/${jobId}`).then((value) => value.job),
  ...
  generateAssistantConfiguration: (tenantId: string, assistantId: string, recommendationId: string, retryRequested = false) => apiClient.post<ConfigurationGenerationResult>(`${tenantRoot(tenantId)}/knowledge-intelligence/assistants/${assistantId}/configurations/generate`, { recommendation_id: recommendationId, ...(retryRequested ? { retry: true } : {}) }),
  getAssistantConfigurationGenerationJob: (tenantId: string, assistantId: string, jobId: string) => apiClient.get<{ job: AssistantConfigurationGenerationJob }>(`${tenantRoot(tenantId)}/knowledge-intelligence/assistants/${assistantId}/configuration-generation-jobs/${jobId}`).then((value) => value.job),
