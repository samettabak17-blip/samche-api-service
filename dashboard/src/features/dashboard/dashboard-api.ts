...
export const tenantApi = {
  ...
  generateImageKnowledgeCandidates: (tenantId: string, sourceId: string, payload: { assistantId?: string | null; extractionHash?: string | null }) => apiClient.post<{ candidates: KnowledgeCandidate[]; reused: boolean; behavior_recommendations?: Array<{ id: string; assistant_id: string; status: string; reused?: boolean }>; warnings?: string[] }>(`${tenantRoot(tenantId)}/knowledge-intelligence/sources/${sourceId}/candidates/generate`, { assistant_id: payload.assistantId ?? null, extraction_hash: payload.extractionHash ?? undefined, candidate_type: 'POLICY' }),
  ...
  generateKnowledgeRecommendation: (tenantId: string, assistantId: string, businessProfileVersionId: string) => apiClient.post<RecommendationGenerationResult>(`${tenantRoot(tenantId)}/knowledge-intelligence/assistants/${assistantId}/recommendations/generate`, { business_profile_version_id: businessProfileVersionId }),
