...
export function AssistantForm({ canManage, initial, onSubmit, isPending = false }: { canManage: boolean; initial?: Assistant; onSubmit(payload: AssistantPayload): void; isPending?: boolean }) {
  ...
  function submit(event: FormEvent) {
    ...
    onSubmit({ name: name.trim(), model: model.trim() || undefined, system_prompt: systemPrompt.trim() || undefined, status });
  }
  return <form onSubmit={submit} className="space-y-4">
    ...
    <label className="block text-sm font-medium text-ink">Model<input aria-label="Model" value={model ?? ''} onChange={(e) => setModel(e.target.value)} className="mt-1.5 w-full rounded-lg border border-line px-3 py-2 text-sm" /></label>
