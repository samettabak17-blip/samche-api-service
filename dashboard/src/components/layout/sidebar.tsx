...
export function Sidebar({ tenantId, tenantName, tenantRole, email, onLogout, onNavigate }: SidebarProps) {
  ...
  return <aside className="flex h-full w-72 flex-col border-r border-white/[0.07] bg-[#0C0C0E] px-4 py-6 text-white">
    ...
    <nav aria-label="Dashboard navigation" className="space-y-1">{navigation.map(({ label, suffix, icon: Icon }) => <NavLink key={suffix} to={`/app/${tenantId}${suffix}`} onClick={onNavigate} className={({ isActive }) => `nav-link ${isActive ? 'nav-link-active' : ''}`}><Icon aria-hidden="true" size={18} strokeWidth={1.8} />{label}</NavLink>)}</nav>
    <div className="mt-auto border-t border-white/10 pt-5"><p className="truncate text-sm font-medium text-stone-300" title={email}>{email}</p><span className="mt-2 inline-flex rounded-md border border-gold/25 bg-gold/10 px-2 py-1 text-[11px] font-semibold uppercase tracking-wide text-gold">{tenantRole ?? 'Workspace'}</span><button type="button" onClick={onLogout} className="mt-4 w-full rounded-lg border border-white/10 px-3 py-2 text-left text-sm text-stone-400 transition hover:border-signal/30 hover:bg-signal/10 hover:text-white">Sign out</button></div>
  </aside>;
}
