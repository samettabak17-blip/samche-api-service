...
export function Topbar({ tenants, selectedTenantId, tenantId, email, systemRole, selectedTenantRole, onCreateTenant, onAdoptTenant, onSelectTenant, onOpenNavigation, onLogout }: TopbarProps) {
  ...
  const resendInvitation = async () => {
    ...
    setCreateError(null);
    try {
      const result = await onboardingApi.resendInvitation(pendingInvitation.tenantId, pendingInvitation.invitationId);
      setPendingInvitation({ tenantId: pendingInvitation.tenantId, invitationId: result.invitation.id });
    } catch { setCreateError('Could not resend the invitation. Please try again.'); }
  };
  const revokeInvitation = async () => {
    if (!pendingInvitation) return;
    setCreateError(null);
    try { await onboardingApi.revokeInvitation(pendingInvitation.tenantId, pendingInvitation.invitationId); setPendingInvitation(null); }
