import { Bell, Building2, CalendarDays, ChevronLeft, ChevronRight, LogOut, Menu, MoreHorizontal, Search } from 'lucide-react';
...
function DateRangeControl() {
  ...
  return <div className="relative hidden xl:block">
    ...
    {open && typeof document !== 'undefined' && createPortal(<section ref={popoverRef} role="dialog" aria-label="Date range" style={{ position: 'fixed', top: overlayPosition.top, left: overlayPosition.left, zIndex: 60 }} className="w-[calc(100vw-2rem)] sm:w-[23rem] max-w-[23rem] rounded-2xl border border-white/[.14] bg-[#09121f]/95 p-3 text-left shadow-[0_22px_60px_rgba(0,0,0,.55),0_0_28px_rgba(212,33,41,.13)] backdrop-blur-2xl">
...
export function Topbar({ tenants, selectedTenantId, tenantId, email, systemRole, selectedTenantRole, onCreateTenant, onAdoptTenant, onSelectTenant, onOpenNavigation, onLogout }: TopbarProps) {
  ...
  const conversationRoute = (channelType?: string) => channelType === 'WEB_CHAT' ? 'web-chat' : channelType === 'SAMCHEGUIDE' ? 'guide' : 'whatsapp';
  ...
  const openWorkspaceAction = (action: () => void) => {
  ...
  return <header className="relative z-50 flex min-h-[4.25rem] items-center justify-between border-b border-line/80 bg-shell/85 px-4 backdrop-blur-xl sm:px-6 lg:px-7">
    ...
    <div className="flex min-w-0 items-center gap-1.5 sm:gap-3 shrink-0">
      ...
      {systemRole === 'OWNER' && <>
        <DashboardButton type="button" variant="secondary" onClick={() => setPlanReviewOpen(true)} className="topbar-action hidden text-xs 2xl:inline-flex">Upgrade requests</DashboardButton>
        ...
        <DashboardButton type="button" variant="outline" onClick={() => { onboardingIdempotencyKey.current = null; setCreateOpen(true); setCreateError(null); setCreateSuccess(false); }} className="topbar-action hidden border-signal/40 bg-signal/10 text-xs text-white hover:bg-signal/20 2xl:inline-flex"><Building2 size={15} />Create company</DashboardButton>
