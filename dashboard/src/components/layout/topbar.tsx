    ...
    <div className="flex min-w-0 items-center gap-1.5 sm:gap-3 shrink-0">
      ...
      <Modal open={workspaceActionsOpen} title="Workspace actions" onClose={() => setWorkspaceActionsOpen(false)} className="max-w-md">
        <div className="mt-5 space-y-4">
          <div className="rounded-xl border border-line bg-elevated/60 p-3"><p className="text-xs font-semibold text-white">{tenants.find((tenant) => tenant.id === selectedTenantId)?.name || 'Workspace'}</p><p className="mt-1 text-[11px] text-stone-400">{displayRole}</p></div>
          <label htmlFor="workspace-action-tenant" className="dashboard-field-label block text-sm font-semibold">Workspace<select id="workspace-action-tenant" aria-label="Selected tenant" value={selectedTenantId} onChange={(event) => { setWorkspaceActionsOpen(false); onSelectTenant(event.target.value); }} className="dashboard-input dashboard-select mt-2 block h-11 w-full rounded-xl border px-3 text-sm"><option value="">Select workspace</option>{tenants.map((tenant) => <option key={tenant.id} value={tenant.id}>{tenant.name}</option>)}</select></label>
