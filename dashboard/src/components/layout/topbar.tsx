...
import { dashboardButtonClass } from '../ui/dashboard-control';
...
function DateRangeControl() {
  ...
  return <div className="relative hidden xl:block">
    <button ref={triggerRef} type="button" onClick={() => { if (!open) requestHeaderOverlayClose(); setOpen((value) => !value); }} aria-expanded={open} className="topbar-range-control group glass-surface inline-flex h-10 items-center gap-2 rounded-xl px-3 text-xs font-semibold transition hover:bg-white/[.055]">
...
export function Topbar({ tenants, selectedTenantId, tenantId, email, systemRole, selectedTenantRole, onCreateTenant, onAdoptTenant, onSelectTenant, onOpenNavigation, onLogout }: TopbarProps) {
  ...
  const submitCreateTenant = async (event: FormEvent) => {
    ...
    if (!name || !firstName || !lastName || !email) { setCreateError('Company name, administrator name, and email are required.'); return; }
    ...
    try {
      ...
      const response = await onboardingApi.createCompanyInvitation({ name, first_name: firstName, last_name: lastName, email }, idempotencyKey);
      ...
      setCompanyName(''); setAdministratorFirstName(''); setAdministratorLastName(''); setAdministratorEmail(''); setCreateOpen(false); setCreateSuccess(true); onboardingIdempotencyKey.current = null;
  ...
  return <header className="relative z-50 flex min-h-[4.25rem] items-center justify-between border-b border-line/80 bg-shell/85 px-4 backdrop-blur-xl sm:px-6 lg:px-7">
    ...
    <div className="flex min-w-0 items-center gap-2 sm:gap-3">
      ...
      {systemRole === 'OWNER' && <><button type="button" onClick={() => { onboardingIdempotencyKey.current = null; setCreateOpen(true); setCreateError(null); setCreateSuccess(false); }} className={`${dashboardButtonClass('outline')} topbar-action border-signal/40 bg-signal/10 text-xs text-white hover:bg-signal/20`}><Building2 size={15} />Create company</button><Modal open={createOpen} title="Create company" onClose={() => { if (!createPending) setCreateOpen(false); }} initialFocusRef={companyNameRef} closeOnEscape={!createPending} className="max-w-lg"><form onSubmit={submitCreateTenant} className="mt-5 space-y-4"><p className="text-sm leading-6 text-stone-300">Create the company and invite its first administrator to set up their account.</p><label className="block text-sm font-medium text-stone-100">Company name<input ref={companyNameRef} aria-label="Company name" value={companyName} onChange={(event) => setCompanyName(event.target.value)} className="mt-2 h-11 w-full rounded-xl border border-white/20 bg-black/20 px-3 text-sm text-white outline-none transition focus:border-signal" /></label><div className="grid gap-4 sm:grid-cols-2"><label className="block text-sm font-medium text-stone-100">First name<input aria-label="First name" value={administratorFirstName} onChange={(event) => setAdministratorFirstName(event.target.value)} className="mt-2 h-11 w-full rounded-xl border border-white/20 bg-black/20 px-3 text-sm text-white outline-none transition focus:border-signal" /></label><label className="block text-sm font-medium text-stone-100">Last name<input aria-label="Last name" value={administratorLastName} onChange={(event) => setAdministratorLastName(event.target.value)} className="mt-2 h-11 w-full rounded-xl border border-white/20 bg-black/20 px-3 text-sm text-white outline-none transition focus:border-signal" /></label></div><label className="block text-sm font-medium text-stone-100">Email<input aria-label="Email" type="email" autoComplete="email" value={administratorEmail} onChange={(event) => setAdministratorEmail(event.target.value)} className="mt-2 h-11 w-full rounded-xl border border-white/20 bg-black/20 px-3 text-sm text-white outline-none transition focus:border-signal" /></label>{createError && <p role="alert" className="rounded-xl border border-red-400/35 bg-red-950/35 px-3 py-2.5 text-sm text-red-100">{createError}</p>}<div className="flex flex-col-reverse gap-3 border-t border-white/[.08] pt-5 sm:flex-row sm:justify-between"><button type="button" disabled={createPending} onClick={() => { setCreateOpen(false); setAssignOpen(true); }} className="rounded-xl border border-white/20 px-4 py-2.5 text-sm font-semibold text-stone-100 transition hover:bg-white/10 disabled:opacity-60">Assign existing customer</button><button type="submit" disabled={createPending} className="rounded-xl bg-signal px-4 py-2.5 text-sm font-semibold text-white shadow-[0_12px_28px_rgba(212,33,41,0.22)] transition hover:bg-[#B81920] disabled:cursor-not-allowed disabled:opacity-60">{createPending ? 'Creating invitation…' : 'Create company & invite administrator'}</button></div></form></Modal>{createSuccess && <span role="status">Invitation created.</span>}{invitationStatuses.isError && <span role="alert" className="text-xs text-red-200">Invitation status is unavailable.</span>}{selectedInvitation && invitationDeliveryLabel && <div className="hidden xl:flex invitation-delivery-status items-center gap-2 rounded-lg border border-line/70 bg-elevated/40 px-2 py-1"><span role="status" className={selectedInvitation.delivery_status === 'DELIVERY_FAILED' ? 'invitation-delivery-label text-[10px] font-semibold text-red-200' : 'invitation-delivery-label text-[10px] font-semibold text-emerald-200'}>{invitationDeliveryLabel}</span>{selectedInvitation.delivery_code && <span className="invitation-delivery-code text-[10px] font-medium text-stone-300">{selectedInvitation.delivery_code}</span>}<button type="button" onClick={() => void resendInvitation({ tenantId, invitationId: selectedInvitation.id })} className={dashboardButtonClass('ghost') + ' topbar-status-action px-2 text-[10px] text-stone-200'}>Resend</button><button type="button" onClick={() => void revokeInvitation({ tenantId, invitationId: selectedInvitation.id })} className={dashboardButtonClass('ghost') + ' topbar-status-action px-2 text-[10px] text-red-200 hover:border-red-400/50 hover:bg-red-500/10'}>Revoke</button></div>}</>}
