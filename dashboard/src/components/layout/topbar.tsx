...
export function Topbar({ tenants, selectedTenantId, email, onSelectTenant, onOpenNavigation, onLogout }: TopbarProps) {
  ...
  return <header className="flex min-h-[4.25rem] items-center justify-between border-b border-line/80 bg-shell/85 px-4 backdrop-blur-xl sm:px-6 lg:px-7">
    ...
    <div className="flex min-w-0 items-center gap-2 sm:gap-3">
      <form onSubmit={(event) => { event.preventDefault(); if (navigationMatches[searchIndex]) openDestination(navigationMatches[searchIndex][2]); }} className="relative hidden xl:block"><Search size={15} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-stone-400" /><input value={search} onFocus={() => setSearchOpen(true)} onChange={(event) => { setSearch(event.target.value); setSearchIndex(0); setSearchOpen(true); }} onKeyDown={(event) => { if (event.key === 'ArrowDown') { event.preventDefault(); setSearchIndex((index) => Math.min(index + 1, Math.max(navigationMatches.length - 1, 0))); } if (event.key === 'ArrowUp') { event.preventDefault(); setSearchIndex((index) => Math.max(index - 1, 0)); } if (event.key === 'Escape') setSearchOpen(false); }} className="h-10 w-48 rounded-xl border border-line bg-black/15 pl-9 pr-3 text-xs text-ink outline-none transition placeholder:text-stone-500 focus:border-signal/50 2xl:w-60" placeholder="Search anything…" aria-label="Search dashboard destinations" />{searchOpen && <section role="listbox" aria-label="Dashboard destinations" className="absolute right-0 top-12 z-[60] w-72 overflow-hidden rounded-xl border border-white/[.14] bg-[#09121f]/95 p-1 shadow-2xl backdrop-blur-2xl">{navigationMatches.length ? navigationMatches.map(([label, group, path], index) => <button key={path} type="button" role="option" aria-selected={index === searchIndex} onMouseDown={(event) => event.preventDefault()} onClick={() => openDestination(path)} className={'flex w-full items-center justify-between rounded-lg px-3 py-2 text-left text-xs transition ' + (index === searchIndex ? 'bg-white/[.09] text-white' : 'text-stone-300 hover:bg-white/[.06]')}><span>{label}</span><span className="text-[10px] text-stone-500">{group}</span></button>) : <p className="px-3 py-2 text-xs text-stone-500">No dashboard destinations</p>}</section>}</form>
