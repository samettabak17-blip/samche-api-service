...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-canvas lg:grid lg:grid-cols-[18rem_1fr]">
      <div className="hidden lg:block"><Sidebar tenantId={selectedTenant.id} tenantName={selectedTenant.name} tenantRole={role} onNavigate={() => undefined} /></div>
      {mobileOpen && (
        <div className="fixed inset-0 z-50 lg:hidden" role="dialog" aria-modal="true" aria-label="Navigation menu">
          ...
          <div className="relative h-full w-72 shadow-2xl">
            ...
            <Sidebar tenantId={selectedTenant.id} tenantName={selectedTenant.name} tenantRole={role} onNavigate={() => setMobileOpen(false)} />
          </div>
        </div>
      )}
