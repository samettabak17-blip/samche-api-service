...
export interface GuideExperienceField { id: string; label: string; description: string; input_type: GuideInputType; required: boolean; options: Array<{ value: string; label: string }>; min: number | null; max: number | null; unit: string; }
export interface GuideToolTerm { field_id: string; kind: 'NUMBER_MULTIPLIER' | 'SELECT_AMOUNT' | 'BOOLEAN_AMOUNT'; multiplier?: number; amount?: number; amounts?: Record<string, number>; }
export interface GuideExperienceData { brand_name: string; assistant_display_name: string; assistant_status_label: string; welcome_title: string; welcome_message: string; input_placeholder: string; launcher_label: string; empty_state_copy: string; logo_url: string | null; avatar_url: string | null; favicon_url: string | null; theme: { primary_color: string; accent_color: string; background_color: string; foreground_color: string; surface_color: string; border_color: string; font_family: 'SYSTEM' | 'INTER' | 'MANROPE' | 'SERIF'; corner_radius: 'SMALL' | 'MEDIUM' | 'LARGE'; density: 'COMPACT' | 'COMFORTABLE' }; layout: { preset: 'PROFESSIONAL' | 'PREMIUM' | 'MINIMAL' | 'CONVERSATIONAL' | 'COMMERCE' | 'SERVICE'; launcher_style: 'PILL' | 'CIRCLE' | 'PANEL'; header_style: 'STANDARD' | 'COMPACT'; panel_style: 'CARD' | 'FLAT' }; modules: { chat: boolean; guide: boolean; calculator: boolean; ctas: boolean }; hero: { title: string; message: string; cta_label: string }; roadmap: { enabled: boolean; title: string; description: string; steps: GuideExperienceField[] }; interactive_tool: { enabled: boolean; title: string; description: string; currency: string; result_label: string; fields: GuideExperienceField[]; calculation: { base_amount: number; terms: GuideToolTerm[] } }; version?: number; }
