...
async function main() {
  ...
  try {
    ...
    console.log(safeResultLine('PASS', 'BUSINESS_IDENTITY_SCOPE', { status: identityAnalysis.status }));
    ...
    const aiReply = await pollUntil({ operation: async () => database.query(`SELECT content FROM conversation_messages WHERE tenant_id = $1 AND conversation_id = $2 AND sender_type = 'ASSISTANT' AND created_at >= $3::timestamptz ORDER BY created_at DESC LIMIT 1`, [sharedTenantId, conversationId, channelStartedAt]), accept: (result) => result.rowCount === 1, reject: () => false, intervalMs: 3000, timeoutMs: 180_000, timeoutCode: 'TASK6_E2E_WHATSAPP_RESPONSE_TIMEOUT' });
    assertCondition(/cobalt lantern/i.test(aiReply.rows[0]?.content ?? ''), 'TASK6_E2E_WHATSAPP_KNOWLEDGE_CONTEXT_MISSING');
