...
export async function processHumanSupportNotificationOutbox({ database = null, deliver, resolveRecipients = null, now = new Date(), tenantId = null } = {}) {
  ...
  try {
    ...
    const tenantFilter = tenantId ? 'o.tenant_id = $1 AND ' : '';
    const claimed = await client.query(
      `SELECT o.id, o.tenant_id, o.conversation_id, o.escalation_id, o.level_order, l.recipient_rule, l.recipient_target
        ...
        WHERE ${tenantFilter}(o.status IN ('PENDING', 'RETRY') OR (o.status = 'PROCESSING' AND o.processing_started_at <= CURRENT_TIMESTAMP - INTERVAL '5 minutes'))
    ...
    try {
      ...
      if (typeof resolveRecipients === 'function') {
        ...
        if (!Array.isArray(recipients) || recipients.length === 0) {
          ...
          await client.query(
            `UPDATE human_support_escalations
                SET next_due_at = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP
              WHERE id = $1 AND tenant_id = $2 AND status IN ('PENDING', 'ACTIVE')`,
            [row.escalation_id, row.tenant_id]
          );
          console.info('PUSH_RECIPIENTS_RESOLVED = tenant=' + String(row.tenant_id).slice(0, 8) + ' rule=' + row.recipient_rule + ' count=0');
          result.noRecipients = (result.noRecipients ?? 0) + 1;
          await client.query('COMMIT');
          return result;
        }
        console.info('PUSH_RECIPIENTS_RESOLVED = tenant=' + String(row.tenant_id).slice(0, 8) + ' rule=' + row.recipient_rule + ' count=' + recipients.length);
      }
