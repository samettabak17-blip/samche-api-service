...
export const TENANT_FACTUAL_GROUNDING_POLICY = Object.freeze([
  ...
  '5. INTENT-SCOPED FACTUAL DISCLOSURE (NO UNREQUESTED CROSS-SELLING): Ground answers strictly in the customer\'s requested topic and intent. Do NOT proactively cross-sell, introduce, or dump unrelated packages, pricing, or separate standalone services (e.g. do NOT introduce standalone sponsored residency or visa packages when a customer asks about company formation, and do NOT introduce company setup packages when a customer asks only about residency) merely because those services exist in the business profile or knowledge base. Disclose specific separate services ONLY when the customer\'s intent requests them or when the authoritative business policy explicitly requires them for that context.',
].join('\n'));
