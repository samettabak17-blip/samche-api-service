...
export async function resolveHumanSupportRecipients({ database, tenantId, conversationId, rule, target = null }) {
  ...
  // (or no assigned operator was resolved), also include eligible tenant operators with
  // active push subscriptions so support requests are never dropped without a real device notification.
  if (rule === 'ASSIGNED_OWNER') {
    let assignedHasSub = false;
    if (assigned && eligible.some((u) => u.id === assigned)) {
      ...
      assignedHasSub = subCheck.rowCount > 0;
    }
    if (!assignedHasSub) {
      const activeSubUsers = await database.query(
        `SELECT u.id, u.system_role, tu.tenant_role
          ...
          WHERE tu.tenant_id = $1 AND (u.status = 'active' OR u.status = 'ACTIVE')
            AND (tu.tenant_role = 'ADMIN' OR u.system_role = 'OWNER')`,
        [tenantId]
      );
