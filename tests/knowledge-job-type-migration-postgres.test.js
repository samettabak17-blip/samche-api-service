...
if (!connectionString) {
  test('PostgreSQL knowledge job type migration contract skipped: TEST_DATABASE_URL not set', { skip: true }, () => {});
} else {
  if (!isSafeTestDatabaseUrl(connectionString)) throw new Error('KNOWLEDGE_JOB_TYPE_POSTGRES_REQUIRES_TEST_DATABASE_URL');
