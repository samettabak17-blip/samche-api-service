...
test('claims only one pending knowledge job with SKIP LOCKED to keep tenant processing isolated', async () => {
  ...
  assert.match(calls[0].sql, /job_type\s+NOT IN\s*\('GENERATE_IMAGE_CANDIDATES',\s*'GENERATE_BUSINESS_PROFILE',\s*'GENERATE_ASSISTANT_RECOMMENDATION',\s*'GENERATE_ASSISTANT_CONFIGURATION'\)/i);
});
