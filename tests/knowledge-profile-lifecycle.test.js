...
test('prepares a durable profile request fingerprint without invoking either provider operation', async () => {
  ...
  const database = { async query(sql) {
    ...
    if (/FROM knowledge_base_documents/i.test(sql)) return { rows: [profileReady({ id: sourceId, title: 'Company facts', content: 'Synthetic Company provides maintenance.', content_hash: 'a'.repeat(64), trusted_identity_ids: [businessIdentityId] })] };
...
test('resolves a canonical fact through its trusted source identity even when its text omits the company name', async () => {
  ...
  const database = { query: async (sql) => {
    ...
    if (/FROM knowledge_base_documents/i.test(sql)) return { rows: [profileReady({ id: candidateSourceId, title: 'Approved event services fact', content: 'Corporate events and gala dinners are available.', content_hash: 'a'.repeat(64), trusted_identity_ids: [businessIdentityId] })] };
...
test('does not inherit a Business Identity from tenant scope when canonical provenance is absent or conflicts', async () => {
  ...
  const database = { query: async (sql) => {
    if (/FROM business_identities/i.test(sql)) return { rows: [{ id: businessIdentityId, display_name: 'Meridian Arc Technologies LLC', normalized_identity: 'meridian arc technologies' }] };
    if (/FROM knowledge_base_documents/i.test(sql)) return { rows: [profileReady({ id: '11111111-1111-4111-8111-111111111111', title: 'Ambiguous fact', content: 'Service information.', content_hash: 'a'.repeat(64), trusted_identity_ids: [otherIdentityId] })] };
    throw new Error(`unexpected SQL ${sql}`);
  } };
...
test('reports a missing explicit source assignment for canonical image facts instead of weakening provenance with text inference', async () => {
  ...
  const database = { query: async (sql) => {
    if (/FROM business_identities/i.test(sql)) return { rows: [{ id: businessIdentityId, display_name: 'Meridian Arc Technologies LLC', normalized_identity: 'meridian arc technologies' }] };
    if (/FROM knowledge_base_documents/i.test(sql)) return { rows: [profileReady({ id: '11111111-1111-4111-8111-111111111111', title: 'Canonical image fact', content: 'Corporate events are available.', content_hash: 'a'.repeat(64), source_type: 'CONVERSATION_CANDIDATE', trusted_identity_ids: [] })] };
    throw new Error(`unexpected SQL ${sql}`);
  } };
...
test('generates a review-only Business Profile from an explicit resolved source scope with exact provenance', async () => {
  ...
  const database = { query: async (sql, params = []) => {
    ...
    if (/FROM knowledge_base_documents/i.test(sql)) return { rows: [profileReady({ id: '11111111-1111-4111-8111-111111111111', title: 'Company', content: 'Approved company facts', content_hash: 'a'.repeat(64) })] };
  ...
  const sourceScopeQuery = calls.find(({ sql }) => /FROM knowledge_base_documents/i.test(sql));
  assert.match(sourceScopeQuery.sql, /id = ANY\(\$2::uuid\[\]\)/i);
  assert.doesNotMatch(sourceScopeQuery.sql, /processing_status = 'READY'|indexing_status = 'READY'/i);
...
test('a selected source identity must match the chosen Business Identity', async () => {
  const database = { query: async (sql) => {
    if (/FROM business_identities/i.test(sql)) return { rows: [{ id: '55555555-5555-4555-8555-555555555555', display_name: 'Meridian Arc Technologies LLC', normalized_identity: 'meridian arc technologies' }] };
    if (/FROM knowledge_base_documents/i.test(sql)) return { rows: [profileReady({ id: '66666666-6666-4666-8666-666666666666', title: 'Nova', content: 'Nova Crest Business Services LLC', content_hash: 'b'.repeat(64) })] };
...
test('identity analysis timeout is classified before profile generation and persists no profile artifact', async () => {
  ...
  const database = { query: async (sql, params = []) => {
    ...
    if (/FROM knowledge_base_documents/i.test(sql)) return { rows: [profileReady({ id: '11111111-1111-4111-8111-111111111111', title: 'Company', content: 'Facts', content_hash: 'a'.repeat(64) })] };
    if (/FROM business_identity_source_evidence/i.test(sql)) return { rows: [] };
    if (/INSERT INTO knowledge_generation_runs/i.test(sql)) return { rows: [{ id: '22222222-2222-4222-8222-222222222222', status: 'RUNNING' }] };
