...
test.before(async () => {
  // The migration replay suite uses this same database and lock. Hold it for
  // this fixture's lifetime so DDL cannot deadlock with its FK-backed rows.
  migrationGuard = await database.connect();
  await migrationGuard.query('SELECT pg_advisory_lock(918246731)');
...
test.after(async () => {
  try {
    ...
    if (migrationGuard) {
      await migrationGuard.query('SELECT pg_advisory_unlock(918246731)');
